...
O procedimento está descrito no algoritmo~\ref{alg:converte1}:
...
\section{Adaptação do Algoritmo~\ref{alg:converte1}}
No algoritmo anterior, só existe um estado de aceitação, assim, para cada derivação do tipo $A \rightarrow \varepsilon$, onde $A \in V$, criamos uma transição do vazia desnecessária ($\delta(A, \epsilon) = \{q_f\}$). Podemos mudar essa abordagem, transfomando $A$ em um estado de aceitação. O algoritmo~\ref{alg:converte2} mostra isso:

\begin{algorithm}[H]
  \caption{$\textsc{Converte}(G)$ (adaptado)}
  \label{alg:converte2}
  \begin{algorithmic}[1]
    \Require Uma GLUD $G = (V, T, P, S)$
    \Ensure Um AFND-$\varepsilon$ $M = (Q, \Sigma, \delta, q_0, F)$,
